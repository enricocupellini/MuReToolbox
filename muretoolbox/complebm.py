...
def complebm(pitches, onsets, durations, method = 'o'):
...
# % See License.txt
    ...
    if not ismonophonic(nmat):
        print(mfilename, ' works only with monophonic input!')
        return None
    
    # Default value for method
    if method not in ['p', 'r', 'o']:
        print('Unknown method. Type ''p'', ''r'' or ''o'' for valid method.')
        return None
    
    # Constant values for normalization
    if method == 'p':
        constant = -0.2407  # mean value of Essen collection is 0
        aveint_ = np.mean(np.diff(pitch(nmat))) * 0.3
        pcd1_ = entropy(pcdist1(nmat)) * 1
        ivd1_ = entropy(ivdist1(nmat)) * 0.8
        ton = tonality(nmat)
        ...
        dud_ = entropy(durdist1(nmat)) * 0.7
        noteden_ = notedensity(nmat, 'sec') * 0.2
        du = dur(nmat, 'sec')
        du = du[du > 0]
        rhyvar_ = np.std(np.log(du)) * 0.5
        metach_ = meteraccent(nmat) * 0.5
        ...
        aveint_ = np.mean(np.diff(pitch(nmat))) * 0.2
        ...
        tonD = d * ton
        ton_ = np.mean(tonD) * -1
        dud_ = entropy(durdist1(nmat)) * 0.5
        noteden_ = notedensity(nmat, 'sec') * 0.4
        du = dur(nmat, 'sec')
        du = du[du > 0]
        rhyvar_ = np.std(np.log(du)) * 0.9
        metach_ = meteraccent(nmat) * 0.8
