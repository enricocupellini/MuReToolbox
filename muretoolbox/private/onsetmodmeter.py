...
"""
    ...
    m = meter(nmat)
    ob = np.mod(onset(nmat), m)
    d = dur(nmat)
    g = np.zeros(4 * m)
    ind = np.mod(np.round(ob * 4), len(g)).astype(int)
    
    for k in range(len(g)):
        g[k] = np.sum(d[ind == k + 1])  # Considera l'indice 1-based in Python
    
    ind2 = np.argmax(g)
    bm = np.mod(onset(nmat) - (ind2) * 0.25, m).T
